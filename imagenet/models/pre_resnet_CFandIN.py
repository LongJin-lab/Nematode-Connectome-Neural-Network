...
class HONet_IN(nn.Module):
    def __init__(self, block, num_blocks, dataset="CIFAR10", k_ini=-9.0 / 5, share_k=True, given_ks=[1.0 / 3, 5.0 / 9, 1.0 / 9, 16.0 / 9],
                 pretrain=False, num_classes=num_cla, stochastic_depth=False,
                 PL=1.0, noise_level=0.001,
                 noise=False):
    ...
    def _make_layer(self, block, planes, num_blocks, stride):
        ...
        #TODO: self.last_res_planes may not be transfered correctly
        #TODO: Why always use a fixed in_planes 64?
        ...
        for stride in strides:
            ...
            #TODO: although it works, it is not reasonable
...
def HONet18_cifar(dataset="CIFAR10", **kwargs):
    return HONet_IN(HOBlock_IN, [2,2,2,2], dataset=dataset, **kwargs)
def honet18_in(dataset="ImageNet", **kwargs):
    return HONet_IN(HOBlock_IN, [2,2,2,2], dataset=dataset, PL=args.PL, k_ini=args.k_ini, given_ks=args.given_ks, **kwargs)
def HONet34_cifar(dataset="CIFAR10", **kwargs):
    return HONet_IN(HOBlock_IN, [3,4,6,3], dataset=dataset, PL=args.PL, k_ini=args.k_ini, given_ks=args.given_ks, **kwargs)
def honet34_in(dataset="ImageNet", **kwargs):
    model = HONet_IN(HOBlock_IN, [3,4,6,3], dataset=dataset, PL=args.PL, k_ini=args.k_ini, given_ks=args.given_ks, **kwargs)
    return model
def honet50_in(dataset = "ImageNet", **kwargs):
    return HONet_IN(HOBottleneck_IN, [3,4,6,3], dataset = dataset, PL=args.PL, k_ini=args.k_ini, given_ks=args.given_ks, **kwargs)
...
# def HONet50_cifar(dataset = "CIFAR10"):
...
def pre_act_resnet18_in(dataset="ImageNet"):
    return PreActResNet(PreActBlock, [2,2,2,2], dataset=dataset, PL=args.PL, k_ini=args.k_ini, given_ks=args.given_ks)
