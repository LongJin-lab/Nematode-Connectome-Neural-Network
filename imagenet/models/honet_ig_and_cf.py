...
#            'pre_resnet152']
__all__ = [ 'honet18_cifar', 'honet50_cifar', 'honet34_cifar', 'honet18_in', 'honet34_in',  'honet50_in', 'pre_act_resnet18_in', 'pre_act_resnet34_in', 'pre_act_resnet50_in', 'pre_act_resnet101_in', 'pre_act_resnet152_in', 'honet101_in', 'honet152_in']
...
class HONet_IN(nn.Module):
    def __init__(self, block, num_blocks, dataset="CIFAR10", k_ini=-9.0 / 5, share_k=False, given_ks=None,
                 pretrain=False, num_classes=num_cla, stochastic_depth=False,
                 PL=1.0, noise_level=0.001,
                 noise=False):
    # def __init__(self, block, num_blocks, dataset="CIFAR10", k_ini=-9.0 / 5, fix_k=True,
...
def honet34_in(dataset="ImageNet", **kwargs):
    return HONet_IN(HOBlock_IN, [3,4,6,3], dataset=dataset, **kwargs)
def honet50_cifar(dataset = "CIFAR10", **kwargs):
...
def honet101_in(dataset = "ImageNet", **kwargs):
    return HONet_IN(HOBottleneck_IN, [3,4,23,3], dataset = dataset, **kwargs)
def honet152_in(dataset = "ImageNet", **kwargs):
    return HONet_IN(HOBottleneck_IN, [3,8,36,3], dataset = dataset, **kwargs)
...
##
    ...
    for name, parameters in net.named_parameters():
        print(name, ':', parameters.size())
    onnx_path = "onnx_model_name_honet18_in.onnx"
    torch.onnx.export(net, d, onnx_path)
    netron.start(onnx_path)
