...
class Model(nn.Module):
    def __init__(self, node_num, p, in_channels, out_channels, graph_mode, graph_type, dataset_mode, is_train):
        ...
        self.SMALL_conv3 = nn.Sequential(
            ConnectomeBlock(self.node_num, self.p, self.in_channels, self.out_channels, self.graph_mode, self.graph_type, self.is_train,
                     name="SMALL_conv3")
        )
        self.SMALL_conv4 = nn.Sequential(
            ConnectomeBlock(self.node_num, self.p, self.in_channels, self.out_channels * 2, self.graph_mode, self.graph_type, self.is_train,
                     name="SMALL_conv4")
        )
        self.SMALL_conv5 = nn.Sequential(
            ConnectomeBlock(self.node_num, self.p, self.in_channels * 2, self.out_channels * 4, self.graph_mode, self.graph_type, self.is_train,
                     name="SMALL_conv5")
        )
